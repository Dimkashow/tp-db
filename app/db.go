...
func AddUserDB(buf User) (err error) {
	_, err = db.Exec(
...
func GetUserByNicknameDB(nickname string) (res *User, err error) {
	user := &User{}
...
func GetUserByEmailDB(email string) (res *User, err error) {
	user := &User{}
...
func AddForumDB(forum Forum) (err error) {
	_, err = db.Exec(
...
func GetForumDB(slug string) (res *Forum, err error) {
	forum := &Forum{}
...
func AddThreadDB(thread Thread) (id uint64, err error) {
	id = 0
	err = db.QueryRow(
...
func getThreadDB(slug string, limit int, since string, desc string) (threads []*Thread, err error) {
...
func getThreadBySlagDB(slug string) (thread *Thread, err error) {
	t := &Thread{}
...
func getThreadByIDDB(id uint64) (thread *Thread, err error) {
	t := &Thread{}
...
func AddPostsDB(posts []*Post) ([]*Post, error) {
...
func addVoteDB(vote Vote) (err error) {
	_, err = db.Exec(
...
func getVoteDB(thread, author uint64) (v *Vote, err error) {
	vote := &Vote{}
...
func getPostFlatDB(id uint64, limit int, since int, desc string) (posts []*Post, err error) {
...
func getPostTreeDB(id uint64, limit int, since int, desc string) (posts []*Post, err error) {
	queryString := "SELECT p.id, p.parent, u.nickname, p.message, p.isEdited, f.slug, p.thread, p.created FROM posts AS p JOIN users AS u ON (p.author = u.id) JOIN forums AS f ON (f.id = p.forum) WHERE p.thread = $1 "
...
func getPostDB(id uint64) (p *Post, err error) {
	post := &Post{}
...
func getPostParentTreeDB(id uint64, limit int, since int, desc string) (posts []*Post, err error) {
...
func editThreadDB(t Thread) {
	_, _ = db.Exec("UPDATE threads SET title = $2, message = $3 WHERE ID = $1", t.ID, t.Title, t.About)
}

func GetForumUsersDB(id uint64, limit uint64, since string, desc string) (users []*User, err error) {
	returnUsers := []*User{}
